  ...
  return (
    <section className="panel">
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-lg font-semibold text-ink">Quality Evaluation</h3>
        <span className="text-xs text-slate-500">Deterministic indicators</span>
      </div>

      {!evaluation ? (
        <p className="rounded-xl border border-dashed border-slate-300 p-4 text-sm text-slate-600">
          Evaluation is optional. Turn on the evaluate toggle when asking a question to generate
          reproducible quality indicators.
        </p>
      ) : (
        <div className="space-y-4">
          ...
          {evaluation.skipped_metrics.length > 0 && (
            <p className="text-xs text-amber-700">
              Skipped metrics: {evaluation.skipped_metrics.join(", ")} (reference data not supplied)
            </p>
          )}
